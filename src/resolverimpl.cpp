...
namespace Sb {
      ...
      public:
            UdpResolver(QueryHandlerIf&handler, const uint16_t requestNo, const std::string&name, const Query::Qtype qType) : handler(handler),
                                                                                                                              requestNo(requestNo), name(name),
                                                                                                                              qType(qType) {
            }
      ...
      void ResolverImpl::cancel(ResolverIf const * /*client*/) {
            std::lock_guard<std::mutex> sync(lock);
            //          auto it = byName.find(&name);
      }

      void ResolverImpl::queryTimedout(uint16_t const queryId) {
      ...
      void ResolverImpl::requestComplete(uint16_t const reqNo, Query::Qanswer const&ans) {
