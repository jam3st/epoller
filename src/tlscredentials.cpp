...
namespace Sb {
    TlsCredentialsManager::TlsCredentialsManager(Botan::RandomNumberGenerator& rng) :
        rng(rng) {
        m_certstores.push_back(new Botan::Certificate_Store_In_Memory(""));
    }

    std::string TlsCredentialsManager::srp_identifier(const std::string& type,
    ...
    bool TlsCredentialsManager::attempt_srp(const std::string& type,
                                            const std::string& hostname) {
    ...
    std::vector<Botan::Certificate_Store*> TlsCredentialsManager::trusted_certificate_authorities(const std::string& type,
                                                                                                  const std::string& /*hostname*/) {
        if(type == "tls-server")
    ...
    void TlsCredentialsManager::verify_certificate_chain(
    ...
    std::string TlsCredentialsManager::srp_password(const std::string& type,
                                                    const std::string& hostname,
    ...
    bool TlsCredentialsManager::srp_verifier(const std::string& /*type*/,
    ...
    std::string TlsCredentialsManager::psk_identity_hint(const std::string&,
                                                         const std::string&)
    {
        return "";
    }

    std::string TlsCredentialsManager::psk_identity(const std::string&, const std::string&,
    ...
    Botan::SymmetricKey TlsCredentialsManager::psk(const std::string& type, const std::string& context,
    ...
    TlsCredentialsManager::load_or_make_cert(const std::string& hostname,
        ...
        if(key_type == "rsa") {
            ...
            key.reset(new ECDSA_PrivateKey(rng, EC_Group("secp521r1")));
        } else {
    ...
    std::vector<Botan::X509_Certificate> TlsCredentialsManager::cert_chain(
        ...
        try {
            if(type == "tls-server") {
                ...
                for(auto i : certs_and_keys)  {
                    ...
                    if(!constTimeValueExists(cert_key_types, i.second->algo_name())) {
                        continue;
                    }
                ...
                std::string key_name = "";
                if(constTimeValueExists(cert_key_types, "RSA")) {
                    key_name = "rsa";
                } else if(constTimeValueExists(cert_key_types, "DSA")) {
                    key_name = "dsa";
                } else if(constTimeValueExists(cert_key_types, "ECDSA")) {
                    key_name = "ecdsa";
                }
    ...
    Botan::Private_Key* TlsCredentialsManager::private_key_for(const Botan::X509_Certificate& cert,
                                                               const std::string& /*type*/,
                                                               const std::string& /*context*/) {
