...
namespace Sb {
      template<typename T> class SyncVec {
            public:
                  SyncVec(T sentinel) : sentinel(sentinel) {
                  ...
                  std::tuple<T, bool> removeAndIsEmpty() {
                        ...
                        if(isEmptyUnlocked()) {
                              ...
                              return std::make_tuple(vec[start++], false);
                        }
                  }
      ...
      template<typename T> const size_t SyncVec<T>::size = sizeof(T);
}
