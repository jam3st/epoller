...
namespace Sb {
      namespace {
            ...
            enum class Rcode {
                  Ok = 0, FormatError = 1, ServerFailure = 2, NameError = 3, NotImplement = 4, Refused = 5
            };
            enum class Opcode {
            ...
            inline size_t getLen(const std::string&name, size_t&prevDotPos) {
            ...
            Footer decodeFooter(const std::vector<uint8_t>&response, size_t&curPos) {
                  Footer footer;
            ...
            Answer decodeAnswer(const std::vector<uint8_t>&response, size_t&curPos) {
                  Answer answer;
            ...
            QueryHeader decodeHeader(const std::vector<uint8_t>&data) {
                  QueryHeader header{};
            ...
            void encodeName(const std::string&name, std::vector<uint8_t>&query) {
                  size_t prevDotPos = 0;
            ...
            std::string decodeName(const std::vector<uint8_t>&response, size_t&curPos) {
                  std::string name;
      ...
      namespace Query {
            ...
            Qanswer decode(Bytes const&data) {
                  ...
                  for (size_t i = header.h.ancount; i > 0; --i) {
                        ...
                        reply.ttl = NanoSecs(answer.ttl * ONE_SEC_IN_NS);
                        reply.timeStamp = SteadyClock::now();
                        if (answer.qclass == Qclass::Internet) {
                              if (answer.rdlength == 4 && answer.qtype == Qtype::A) {
                                    IpAddr addr;
                                    addr.setIpV4(Bytes(data.begin() + curPos, data.begin() + curPos + sizeof(uint32_t)));
                                    curPos += sizeof(uint32_t);
                                    logDebug("Adding XXXXXXXXXXXXXXXXXXXXX without timeout " + addr.toString());
                                    reply.addr.push_back(addr);
                                    reply.valid = true;
                              } else if (answer.rdlength == 16 && answer.qtype == Qtype::Aaaa) {
                                    IpAddr addr;
                                    std::array<uint8_t, IP_ADDRESS_BIT_LEN / NUM_BITS_PER_SIZE_T> netOrder;
                                    for (size_t i = 0; i < sizeof(netOrder); ++i) {
                                          netOrder[i] = data[curPos + i];
                                    }
                                    addr.set(netOrder);
                                    curPos += sizeof(netOrder);
                                    logDebug("Adding XXXXXXXXXXXXXXXXXXXXX without timeout " + addr.toString());
                                    reply.addr.push_back(addr);
                                    reply.valid = true;
                              }
